...
/obj/effect/temp_visual/goliath_tentacle/proc/trip()
	...
	for(var/mob/living/L in loc)
		...
		if(!L.has_anus(TRUE))
			...
			visible_message("<span class='danger'>[src] slides into [L]'s ass!</span>")
	for(var/mob/living/carbon/human/H)
		if(H.has_anus(TRUE))
			H.adjust_arousal(20)
			visible_message("<span class='danger'>[src] slides into [H]'s ass!</span>")
		if(H.has_balls(TRUE))
			H.adjust_arousal(20)
			visible_message("<span class='danger'>[src] slides around [H]'s balls!</span>")
		if(H.has_breasts(TRUE))
			H.adjust_arousal(20)
			visible_message("<span class='danger'>[src] titfucks [H]!</span>")
		if(H.has_vagina(TRUE))
			H.adjust_arousal(20)
			visible_message("<span class='danger'>[src] fuckes [H]'s vagina!</span>")
		if(H.has_penis(TRUE))
			H.adjust_arousal(20)
			visible_message("<span class='danger'>[src] jacks [H] off!</span>")
